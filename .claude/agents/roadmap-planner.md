...
## Phase 번호 체계 규칙 (MANDATORY)

### 순차 번호만 사용
- Phase 1, Phase 2, Phase 3, Phase 4... 순차 정수만 사용한다
- **서브 Phase(2A, 2B, 3B 등) 생성 금지** — 각 Phase는 독립된 테마를 가진다
- 각 Phase 헤더에 테마를 명시한다: `## Phase N: {테마명}`

### Phase 테마 원칙
- 하나의 Phase = 하나의 명확한 테마/목표
- 예: "Phase 2: OCR + PDF 압축", "Phase 3: Tauri 데스크탑"
- 테마가 다른 기능은 별도 Phase로 분리한다

### 새 요구사항 배치
1. 미착수 Phase 중 테마가 맞는 곳이 있으면 → 그 Phase에 추가
2. 맞는 테마가 없으면 → 마지막 Phase 뒤에 새 Phase N+1 생성
3. **절대 하지 않는 것**: 완료된 Phase에 기능 추가, 기존 Phase 번호 변경

## Phase 상태 관리 규칙 (MANDATORY)

### Phase 상태 정의

| 상태 | ROADMAP 표기 | 의미 |
|------|-------------|------|
| **완료** | 헤더에 `✅ 완료` 또는 완료 기준 체크박스 전체 `[x]` | 불변 — 수정 금지 |
| **진행중** | 헤더에 `🔄 진행중` 또는 shrimp-task-manager에 태스크 등록됨 | 범위 추가만 허용 |
| **미착수** | 상태 표기 없음 또는 `(향후 검토)` | 자유롭게 수정 가능 |

### 불변성 규칙

1. **완료 Phase**: 절대 수정 금지 (유일한 예외: 오타 수정)
2. **진행중 Phase**: 새 태스크 추가 가능, 기존 태스크 삭제/재작성 금지
3. **미착수 Phase**: 자유롭게 수정 가능
...
### notes 필드에 도메인 메타데이터
```
domain: markdown | phase: 1 | priority: must-have
```
...
### 도메인별 조회 방법
```
mcp__shrimp-task-manager__query_task(keyword="[Markdown]")
→ Markdown 도메인 태스크만 필터
```
...
1. **도메인 ROADMAP 읽기**: `docs/{domain}/ROADMAP.md`의 "Phase N 태스크 정의" 블록 읽기
...
0. **Phase 상태 확인**: ROADMAP의 각 Phase 상태(완료/진행중/미착수)를 파악한다
   - 헤더에 `✅ 완료` 또는 완료 기준 전부 `[x]` → 완료
   - shrimp-task-manager에 해당 Phase 태스크 등록됨 → 진행중
   - 그 외 → 미착수
...
- `[Markdown] Phase1: 마크다운 에디터 UI 화면 구현` (Figma 에디터 화면 참조)
