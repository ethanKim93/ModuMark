...
4. BRD 구성 요소를 순서대로 채워 `docs/BRD.md`를 작성한다.
5. ddd-architect가 도메인을 분리한 이후, 도메인별 비즈니스 요구사항을 `docs/{domain}/BRD.md`에 작성한다.
   - 전체 BRD에서 해당 도메인과 관련된 목표, 이해관계자, KPI, 제약조건을 추출하여 상세화한다.
   - 도메인 고유의 비즈니스 규칙(Business Rule)을 별도로 정의한다.
6. 작성 후 PRD 작성을 위한 핵심 비즈니스 요구사항 요약을 제공한다.
