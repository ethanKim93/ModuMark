...
import type { MarkdownEditorMode } from '../ModeToggle';

describe('MarkdownEditorMode 전환 로직', () => {
  it('WYSIWYG → Raw 전환 시 마크다운 내용이 동일하다', () => {
    ...
    const modeAfterToggle: MarkdownEditorMode = 'raw';
  ...
  it('Raw 편집 후 WYSIWYG 전환 시 수정된 내용이 반영된다', () => {
    ...
    const modeAfterToggle: MarkdownEditorMode = 'wysiwyg';
    expect(modeAfterToggle).toBe('wysiwyg');
    expect(storedContent).toBe(editedContent);
  });

  it('모드 토글 함수가 올바른 값을 반환한다', () => {
    const toggle = (current: MarkdownEditorMode): MarkdownEditorMode =>
  ...
  it('탭 전환 시 모드가 wysiwyg로 초기화된다', () => {
    let mode: MarkdownEditorMode = 'raw';
  ...
  it('마크다운 내용이 빈 문자열이어도 모드 전환이 가능하다', () => {
    ...
    const mode: MarkdownEditorMode = 'raw';
