...
export function PdfThumbnailList() {
  ...
  const handleClick = (id: string) => {
    ...
    const { activeFile, files, setActiveFileAndPage } = usePdfFileStore.getState();
    if (activeFile && pageItem.fileName !== activeFile.name) {
      // 다른 파일 페이지: 파일+페이지+viewerStates 원자적 전환 (savedPage 오염 방지)
      const fileItem = files.find((f) => f.id === pageItem.fileId);
      if (fileItem) {
        setActiveFileAndPage(fileItem.file, pageItem.pageIndex, pageItem.id);
        return;
      }
    }
